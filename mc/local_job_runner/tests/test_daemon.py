...
class ProcessCandidateJobSpecTestCase(DaemonBaseTestCase):
    ...
    def test_claimable_job_spec(self):
        ...
        expected_job_proc_meta = self.mocks['start_job_run'].return_value
        ...
        self.assertEqual(self.mocks['start_job_run'].call_args,
                         call(job=expected_partial_job))
        expected_full_job = {**expected_partial_job,
                             'proc': expected_job_proc_meta}
        self.assertEqual(self.daemon.running_jobs[expected_partial_job['key']],
                         expected_full_job)
    ...
    def test_unclaimable_job_spec(self):
        ...
        self.assertEqual(self.mocks['start_job_run'].call_count, 0)
...
class RunJobTestCase(DaemonBaseTestCase):
    ...
    def test_start_job_run(self):
        ...
        proc_meta = self.daemon.start_job_run(job=job)
...
class ProcessRunningJobsTestCase(DaemonBaseTestCase):
    def setUp(self):
        super().setUp()
        self.completed_jobs = [{'key': 'completed_%s' % i} for i in range(3)]
        self.running_jobs = [{'key': 'running_%s' % i} for i in range(3)]
        self.daemon.running_jobs = {
            j['key']: j for j in self.completed_jobs + self.running_jobs
        }
        self.mock_job_run_states = {
            **{j['key']: None for j in self.completed_jobs},
            **{j['key']: {'mock': 'state'} for j in self.running_jobs}
        }
        self.patcher = patch.multiple(self.daemon,
                                      get_job_run_states=DEFAULT,
                                      process_completed_job=DEFAULT)
        self.mocks = self.patcher.start()
        self.mocks['get_job_run_states'].return_value = \
                self.mock_job_run_states

    def tearDown(self):
        self.patcher.stop()

    def test_process_running_jobs(self):
...
class ProcessCompletedJobTestCase(DaemonBaseTestCase):
    def setUp(self):
        ...
        self.mocks = self.patcher.start()
        self.job_key = 'abcd'
        self.completed_job = {'key': self.job_key}
        self.daemon.running_jobs[self.job_key] = self.completed_job
    ...
    def test_transfers_completed_jobs(self):
        ...
        self.assertTrue(self.job_key not in self.daemon.running_jobs)
        self.assertEqual(
...
class GetJobRunStatesTestCase(DaemonBaseTestCase):
    def setUp(self):
        ...
        for i in range(3):
            ...
            self.daemon.running_jobs[job_key] = {'key': job_key,
                                                 'proc': {'pid': i}}

    def tearDown(self):
        self.patcher.stop()

    def test_get_job_run_states(self):
        job_run_states = self.daemon.get_job_run_states()
        ...
        mock_job_run_state = operator.attrgetter(
            'return_value.as_dict.return_value')(self.mocks['psutil.Process'])
        expected_job_states = {job_key: mock_job_run_state
                               for job_key in self.daemon.running_jobs.keys()}
        self.assertEqual(job_run_states, expected_job_states)
