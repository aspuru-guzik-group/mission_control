...
class AddTaskTestCase(BaseTestCase):
    ...
    def test_add_as_root(self):
        ...
        self.assertEqual(self.flow.root_task_key, self.task['key'])
...
class AddEdgeTestCase(BaseTestCase):
    ...
    def test_updates_edges_by_task_key(self):
        ...
        self.flow.add_edge(edge=self.edge)
        self.assertEqual(
            self.flow.edges_by_task_key[self.edge['src_key']]['outgoing'],
            {self.expected_edge_key: self.edge})
        self.assertEqual(
            self.flow.edges_by_task_key[self.edge['dest_key']]['incoming'],
            {self.expected_edge_key: self.edge})
...
class GetSuccessorsTestCase(BaseTestCase):
    ...
    def setup_flow(self, flow=None):
        ...
        self.flow.add_task(task_1, precursor_keys=[flow.root_task_key])
        task_1_1 = self.generate_task(key='1_1')
        self.flow.add_task(task_1_1, precursor_keys=[task_1['key']])
        task_1_2 = self.generate_task(key='1_2')
        self.flow.add_task(task_1_2, precursor_keys=[task_1['key']])
