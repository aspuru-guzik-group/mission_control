...
class ChemThingActionProcessor(object):
    ...
    def process_action(self, action=None):
        chemthing, created = self.get_or_create_chemthing_for_action(
            action=action)
        return self.update_chemthing_from_action(chemthing=chemthing,
                                                 action=action)

    def get_or_create_chemthing_for_action(self, action=None):
        filter_kwargs = {}
        if 'key' in action: filter_kwargs['keys__has_key'] = action['key']
