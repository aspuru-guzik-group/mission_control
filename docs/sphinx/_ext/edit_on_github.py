...
def html_page_context(app, pagename, templatename, context, doctree):
    ...
    if not app.config.edit_on_github_repo_root_abs_path:
        warnings.warn("edit_on_github_repo_root_abs_path not specified")
        return

    path = os.path.relpath(doctree.get('source'),
                           app.config.edit_on_github_repo_root_abs_path)
...
def setup(app):
    ...
    app.add_config_value('edit_on_github_repo_root_abs_path', None, True)
