...
class ListFlowsTestCase(BaseTestCase):
    def test_list_flows(self):
        ...
        expected_data = [_serializers.FlowSerializer(flow).data
                         for flow in flows]
        self.assertEqual(key_by_key(response.data), key_by_key(expected_data))

    def test_status_filtering(self):
        statuses = [Statuses.PENDING, Statuses.RUNNING]
        flows_by_status = {
        ...
        for status in statuses:
            ...
            self.assertEqual(key_by_key(response.data),
                             key_by_key(expected_data))
    ...
    def test_key_filtering(self):
        ...
        for key, flow in flows_by_key.items():
            response = self.client.get(BASE_URL + 'flows/', {'key': key})
            expected_data = [_serializers.FlowSerializer(flow).data]
            self.assertEqual(key_by_key(response.data),
                             key_by_key(expected_data))
...
class TickableFlowsTestCase(BaseTestCase):
    ...
    def generate_flow_groups(self):
        flow_groups = {
            'tickable': [
                _models.Flow.objects.create(claimed=True, status=status)
                for status in Statuses.tickable_statuses
            ],
            'untickable': [
                _models.Flow.objects.create(
                    claimed=True, status=Statuses.COMPLETED)
            ],
        }
...
class PatchFlowTestCase(BaseTestCase):
    ...
    def test_patch_flow(self):
        ...
        new_values = {'status': Statuses.RUNNING}
        response = self.client.patch(
            BASE_URL + 'flows/%s/' % flow_to_patch.key, new_values)
        self.assertEqual(response.status_code, 200)
        patched_flow = _models.Flow.objects.get(key=flow_to_patch.key)
...
class ClaimFlowTestCase(BaseTestCase):
    ...
    def _claimFlows(self):
        keys = [flow.key for flow in self.flows_to_claim]
    ...
    def test_db_state(self):
        ...
        for flow in self.all_flows:
            ...
            expected_claimed_values[flow.key] = expected_claimed_value
        self.assertEqual(claimed_values, expected_claimed_values)


class ListJobsTestCase(BaseTestCase):
    def test_list_jobs(self):
        jobs = [_models.Job.objects.create(label="j_%s" % i) for i in range(3)]
        response = self.client.get(BASE_URL + 'jobs/')
        expected_data = [_serializers.JobSerializer(job).data for job in jobs]
        self.assertEqual(sorted(response.data, key=lambda j: j['key']),
                         sorted(expected_data, key=lambda j:j['key']))
    ...
    def test_key_filtering(self):
        ...
        for key, job in jobs_by_key.items():
            response = self.client.get(BASE_URL + 'jobs/', {'key': key})
            expected_data = [_serializers.JobSerializer(job).data]
            self.assertEqual(self.key_by_key(response.data),
                             self.key_by_key(expected_data))

    def key_by_key(self, items=None):
        return {item['key']: item for item in items}
...
class PatchJobTestCase(BaseTestCase):
    ...
    def test_patch_job(self):
        ...
        new_values = {'label': 'new_label',
                      'serialization': json.dumps({'new': 'serialization'})}
        response = self.client.patch(BASE_URL + 'jobs/%s/' % job_to_patch.key,
                                     new_values)
        self.assertEqual(response.status_code, 200)
        patched_job = _models.Job.objects.get(key=job_to_patch.key)
...
class ClaimJobTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.claimed_jobs = [_models.Job.objects.create(
            label="job_%s" % i, status=Statuses.RUNNING)
            for i in range(1)]
    ...
    def _claimJobs(self):
        keys = [job.key for job in self.jobs_to_claim]
    ...
    def test_response_data(self):
        self.assertEqual(self.response.status_code, 200)
        expected_data = {str(job.key): _serializers.JobSerializer(job).data
                         for job in self.jobs_to_claim}
        self.assertEqual(json.loads(self.response.content.decode()),
                         expected_data)

    def test_db_state(self):
        statuses = {job.key: job.status for job in _models.Job.objects.all()}
        expected_statuses = {}
        for job in self.all_jobs:
            ...
            expected_statuses[str(job.key)] = expected_status
...
class QueueItemsTestCase(BaseTestCase):
    ...
    @patch.object(_views, 'DjDao')
    def test_returns_claimed_queue_items(self, _DjDao):
        expected_dao = _DjDao.return_value
        expected_dao.claim_queue_items.return_value = {'mock': 'items'}
        url = '{base_url}queues/{queue_key}/claim_items/'.format(
            base_url=BASE_URL, queue_key=self.queue_key)
        response = self.client.post(url)
        self.assertEqual(
            _DjDao.call_args,
            call(models=self.expected_models,
                 serializers=self.expected_serializers)
        )
        self.assertEqual(response.status_code, 200)
        result = response.json()
        self.assertEqual(
            expected_dao.claim_queue_items.call_args,
            call(queue_key=self.queue_key)
        )
        self.assertEqual(result['items'],
                         expected_dao.claim_queue_items.return_value)
